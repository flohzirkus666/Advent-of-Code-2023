...
const PART_ONE: &str = "Game 1: 10 green, 9 blue, 1 red; 1 red, 7 green; 11 green, 6 blue; 8 blue, 12 green
...
fn extract_value(items: Vec<&str>) -> u8 {
    // counting each element
    let mut number: u8 = 0;
    let mut num_str: &str;
    let re = Regex::new(r"\d{1,}").unwrap();
    for item in items {
        num_str = re.find(item).unwrap().as_str();
        number += num_str.parse::<u8>().unwrap();
    }
    number
}

fn get_cubes(line: &str) -> Option<u8> {
    // Split a line into single elements

    let mut color_blue: u8 = 0;
    let mut color_green: u8 = 0;
    let mut color_red: u8 = 0;

    let id = Regex::new(r"Game\s\d{1,}:").unwrap();
    let game: Vec<&str> = id.find_iter(line).map(|m| m.as_str()).collect();
    let id_num = extract_value(game);
    let mut overreached = false;

    for game in line.split(";").into_iter() {
        let blue = Regex::new(r"\d{1,}\sblue").unwrap();
        let red = Regex::new(r"\d{1,}\sred").unwrap();
        let green = Regex::new(r"\d{1,}\sgreen").unwrap();

        for (index, color) in [blue, red, green].iter().enumerate() {
            let color_str: Vec<&str> = color.find_iter(game).map(|m| m.as_str()).collect();
            let result = extract_value(color_str);
            match index {
                0 => color_blue = result,
                1 => color_red = result,
                2 => color_green = result,
                _ => panic!("Kaboom rico!"),
            };
        }
        if color_red > 12 || color_green > 13 || color_blue > 14 {
            overreached = true;
            break;
        }
    }
...
fn main() {
    ...
    let games: Vec<&str> = PART_ONE.split("\n").collect();

    let mut possible_games: u32 = 0;

    for game in games {
        let bag = get_cubes(game);
        // A game is possible when 12 red cubes, 13 green cubes, and 14 blue cubes
        match bag {
            Some(x) => possible_games += x as u32,
            None => continue,
        }
    }
    println!("Sum of possible games: {possible_games}");
}
